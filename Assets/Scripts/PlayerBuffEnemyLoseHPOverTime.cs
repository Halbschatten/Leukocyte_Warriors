...
public class PlayerBuffEnemyLoseHPOverTime : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == playerTag)
        {
            ...
            {
                gameControllerScript.Players[i].GetComponent<PlayerScript>().BuffEnemyLoseHPOverTime(duration, multiplier);
                //Debug.Log(string.Format("[Player {0}]: [{1}, {2:00}s, {3:#.##}x] activated!", i + 1, "buffEnemyLoseHPOverTime", duration, multiplier));
            }
