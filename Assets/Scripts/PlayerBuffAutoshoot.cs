...
public class PlayerBuffAutoshoot : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == playerTag)
        {
            ...
            {
                gameControllerScript.Players[i].GetComponent<PlayerScript>().BuffAutoShoot(duration);
                //Debug.Log(string.Format("[Player {0}]: [{1}, {2:00}s, {3:#.##}x] activated!", i + 1, "buffAutoShoot", duration, multiplier));
            }
